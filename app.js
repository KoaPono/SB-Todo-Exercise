...
taskList.addEventListener("click", function(e) {
    if (e.target.tagName === "BUTTON") {
        const desiredId = e.target.previousSibling.firstChild.id;
...
function createTask(taskName, isChecked) {
    ...
    taskCheckbox.classList.add("sul-checkbox-type-1");
    ...
    taskRemoveButton.classList.add("sul-btn");
    taskRemoveButton.innerText = "-";

    // Add all elements to wrapper div and add wrapper div to list
    taskNameDiv.append(taskCheckbox);
    taskNameDiv.append(taskLabel);
    taskDiv.append(taskNameDiv);
